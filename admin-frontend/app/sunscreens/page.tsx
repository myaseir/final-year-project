...
export default function SunscreenSelectionPage() {
  ...
  const handleSelect = async (productId: string, productName: string, price: number) => {
    ...
    try {
      // Pull the base URL from your environment variables
      const baseUrl = process.env.NEXT_PUBLIC_API_URL;
      
      // Safety check to ensure the URL is defined
      if (!baseUrl) {
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col bg-[#fff2e8] text-gray-900 font-sans relative">
      ...
      <header className="relative flex justify-between items-center px-6 py-4">
        ...
        <Link href="/" className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center top-3 hover:opacity-80 transition cursor-pointer z-10">
          ...
          <span className="text-xl tracking-[0.15em] font-light text-orange-900">VEND CARE</span>
        </Link>

        {/* Right Actions */}
        <div className="flex-1 flex justify-end gap-6 relative z-10">
          <button className="flex flex-col items-center text-orange-900 hover:text-black transition">
      ...
      <main className="flex-grow flex flex-col items-center pt-8 px-6 pb-20 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full justify-center relative">
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 transition-opacity duration-300 ${showSuccess ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
            ...
            {/* Product 1: SunGuard */}
            <div className="w-[240px] bg-[#fdf5f0] border border-[#f0dac8] rounded-2xl p-4 flex flex-col shadow-sm hover:shadow-md transition">
              <div className="h-40 flex justify-center items-center mb-4">
                <svg width="60" height="100" viewBox="0 0 60 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15 10 H45 L40 80 H20 Z" fill="#f9c851"/>
                  <path d="M15 10 H45 V5 H15 Z" fill="#ffffff"/>
                  <path d="M22 80 H38 L40 95 H20 Z" fill="#ffffff"/>
                  <text x="30" y="35" fontSize="8" fill="#d97743" textAnchor="middle" fontWeight="bold">SUN</text>
                  <text x="30" y="45" fontSize="8" fill="#d97743" textAnchor="middle" fontWeight="bold">GUARD</text>
                  <line x1="20" y1="65" x2="40" y2="65" stroke="#ffffff" strokeWidth="2"/>
                </svg>
              </div>
              <h3 className="text-lg font-bold text-black leading-tight">SunGuard</h3>
              <p className="text-black/80 text-sm mb-2">SPF 30 Daily Lotion</p>
              <p className="text-xl font-bold mb-3">₹40</p>
              <button 
                onClick={() => handleSelect('prod_sun_01', 'SunGuard', 40)}
                disabled={loadingProduct === 'prod_sun_01'}
                className="mt-auto w-full bg-[#e88a4e] text-white py-2 rounded-xl font-medium tracking-wide hover:bg-[#d47234] transition disabled:opacity-70"
              >
                {loadingProduct === 'prod_sun_01' ? 'LOADING...' : 'SELECT'}
              </button>
            </div>
            ...
            <div className="w-[240px] bg-[#fdf5f0] border border-[#f0dac8] rounded-2xl p-4 flex flex-col shadow-sm hover:shadow-md transition">
              <div className="h-40 flex justify-center items-center mb-4">
                <svg width="60" height="100" viewBox="0 0 60 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="18" y="30" width="24" height="60" rx="4" fill="#e88a4e"/>
                  <path d="M22 30 C22 20, 38 20, 38 30 Z" fill="#ffffff"/>
                  <rect x="25" y="15" width="10" height="10" fill="#f9c851"/>
                  <path d="M25 15 H35 V10 H25 Z" fill="#ffffff"/>
                  <path d="M20 15 H25 V12 H20 Z" fill="#ffffff"/>
                  <text x="30" y="55" fontSize="8" fill="#ffffff" textAnchor="middle" fontWeight="bold">UV</text>
                  <text x="30" y="65" fontSize="8" fill="#ffffff" textAnchor="middle" fontWeight="bold">SHIELD</text>
                </svg>
              </div>
              <h3 className="text-lg font-bold text-black leading-tight">UV Shield</h3>
              <p className="text-black/80 text-sm mb-2">SPF 50 Spray</p>
              <p className="text-xl font-bold mb-3">₹60</p>
              <button 
                onClick={() => handleSelect('prod_sun_02', 'UV Shield', 60)}
                disabled={loadingProduct === 'prod_sun_02'}
                className="mt-auto w-full bg-[#e88a4e] text-white py-2 rounded-xl font-medium tracking-wide hover:bg-[#d47234] transition disabled:opacity-70"
              >
                {loadingProduct === 'prod_sun_02' ? 'LOADING...' : 'SELECT'}
              </button>
            </div>
            ...
            <div className="w-[240px] bg-[#fdf5f0] border border-[#f0dac8] rounded-2xl p-4 flex flex-col shadow-sm hover:shadow-md transition">
              <div className="h-40 flex justify-center items-center mb-4">
                 <svg width="60" height="100" viewBox="0 0 60 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M20 30 C20 25, 40 25, 40 30 L45 90 C45 95, 15 95, 15 90 Z" fill="#6ba7c6"/>
                  <rect x="23" y="18" width="14" height="12" fill="#e88a4e"/>
                  <path d="M23 18 H37 V13 C37 10, 23 10, 23 13 Z" fill="#ffffff"/>
                  <text x="30" y="55" fontSize="8" fill="#ffffff" textAnchor="middle" fontWeight="bold">Aqua</text>
                  <text x="30" y="65" fontSize="8" fill="#ffffff" textAnchor="middle" fontWeight="bold">Sun</text>
                </svg>
              </div>
              <h3 className="text-lg font-bold text-black leading-tight">Aqua Sun</h3>
              <p className="text-black/80 text-sm mb-2">Water Resistant</p>
              <p className="text-xl font-bold mb-3">₹55</p>
              <button 
                onClick={() => handleSelect('prod_sun_03', 'Aqua Sun', 55)}
                disabled={loadingProduct === 'prod_sun_03'}
                className="mt-auto w-full bg-[#e88a4e] text-white py-2 rounded-xl font-medium tracking-wide hover:bg-[#d47234] transition disabled:opacity-70"
              >
                {loadingProduct === 'prod_sun_03' ? 'LOADING...' : 'SELECT'}
              </button>
            </div>
          ...
          <div className={`w-[300px] h-fit bg-[#ffd8b8] rounded-2xl flex flex-col items-center pt-6 pb-8 shadow-sm transition-opacity duration-300 ${showSuccess ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
            ...
            <div className="relative mb-4 bg-[#ffd8b8] w-[140px] h-[140px] flex items-center justify-center">
