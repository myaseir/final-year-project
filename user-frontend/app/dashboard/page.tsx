...
export default function Dashboard() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-white p-20 text-center flex flex-col items-center gap-4">
      ...
      <div className="max-w-5xl mx-auto p-6 md:p-10 text-[#4A3F3F]">
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <div className="flex flex-wrap gap-4 mt-3 text-[9px] md:text-[10px] font-bold text-[#8C7A7A] uppercase tracking-[0.2em]">
          ...
          <div className="bg-white border border-[#F9EAEA] px-5 py-3 rounded-2xl shadow-sm w-fit">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16">
          ...
          {/* Modern Balance Card */}
          <div className="bg-[#4A3F3F] text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
            <div className="absolute -right-6 -top-6 h-32 w-32 bg-white/10 rounded-full blur-2xl group-hover:bg-[#E29595]/20 transition-all duration-700"></div>
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <h2 className="text-4xl md:text-5xl font-bold tracking-tighter">{user.balance.toLocaleString()}</h2>
            </div>
          ...
          <div className="md:col-span-2 bg-white border border-[#F9EAEA] p-8 rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between gap-8 relative overflow-hidden">
             ...
             <div className="flex flex-col sm:flex-row items-center gap-4 z-10">
               <button 
                 ...
                 className="bg-[#E29595] text-white px-10 py-4 rounded-full text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-[#D48484] transition-all shadow-xl shadow-[#E29595]/20 active:scale-95"
               >
                 Request Top-up
               </button>
        ...
        <section>
          <div className="flex items-center justify-between mb-8 px-2 md:px-4">
            <div>
              ...
              <p className="text-[10px] text-[#8C7A7A] uppercase tracking-widest font-bold mt-1">Vending Unit Logs</p>
            </div>
          ...
          <div className="bg-white border border-[#F9EAEA] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#F9EAEA] text-[#E29595] text-[10px] uppercase tracking-[0.2em] font-bold">
                    <th className="p-6 md:p-8">Product Details</th>
                    <th className="p-6 md:p-8 hidden sm:table-cell">Location ID</th>
                    <th className="p-6 md:p-8 hidden md:table-cell">Timestamp</th>
                    <th className="p-6 md:p-8 text-right">Debit</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#FDF8F8]">
                  {user.history && user.history.length > 0 ? (
                    user.history.map((item, index) => (
                      <tr key={index} className="hover:bg-[#FDF8F8] transition-all group">
                        <td className="p-6 md:p-8">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                               ...
                               <span className="text-[9px] text-[#F3C5C5] uppercase tracking-widest font-bold">Care Collection</span>
                               <span className="md:hidden block text-[8px] text-[#8C7A7A] mt-1">{item.date}</span>
                             </div>
                          </div>
                        </td>
                        <td className="p-6 md:p-8 text-xs text-[#8C7A7A] font-medium tracking-wider hidden sm:table-cell">{item.machine_id}</td>
                        <td className="p-6 md:p-8 text-xs text-[#8C7A7A] font-medium tracking-wide hidden md:table-cell">{item.date}</td>
                        <td className="p-6 md:p-8 text-right font-bold text-[#4A3F3F] tabular-nums">- {item.amount} <span className="text-[10px] font-light">PKR</span></td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={4} className="p-20 text-center bg-white">
                        <div className="flex flex-col items-center gap-3 opacity-40">
                          ...
                          <p className="italic font-serif text-[#8C7A7A]">No transactions recorded yet.</p>
                        </div>
                      </td>
                    </tr>
                  )}
